# based on https://github.com/mitmul/chainer-fast-rcnn/blob/master/functions/roi_pooling_2d.py
# CPU mode added by R. Yamauchi (2016)
...
class ROIPooling2D(function.Function):
        ...
        for roi_n in xrange(n_rois):
            ...
            for ph in range(self.pooled_height):
                for pw in range(self.pooled_width):
                    hstart = xp.floor(bin_size_h * ph)
                    wstart = xp.floor(bin_size_w * pw)
                    hend = xp.floor(bin_size_h * (ph+1))
                    wend = xp.floor(bin_size_w * (pw+1))
                    ...
                    if not is_empty:
                        y[roi_n, :, ph, pw] = xp.max(x[:, :, hstart:hend, wstart:wend], axis=(2,3))
        return y,
        ...
        for roi_n in xrange(n_rois):
            ...
            for ph in range(self.pooled_height):
                for pw in range(self.pooled_width):
                    ...
                    is_empty = (hend <= hstart) or (wend <= wstart)
                    if not is_empty:
                        gx[:, :, hstart:hend, wstart:wend] += gy[roi_n, :, ph:ph+1, pw:pw+1]
        return gx, None
    ...
    def forward_gpu(self, inputs):
        ...
        top_data = cuda.cupy.empty((n_rois, channels, self.pooled_height,
                                    self.pooled_width), dtype=np.float32)
        self.argmax_data = cuda.cupy.empty_like(top_data).astype(np.int32)
    ...
    def backward_gpu(self, inputs, gy):
        bottom_data, bottom_rois = inputs
        channels, height, width = bottom_data.shape[1:]
        bottom_diff = cuda.cupy.zeros_like(bottom_data, dtype=np.float32)
        cuda.cupy.ElementwiseKernel(
            '''
